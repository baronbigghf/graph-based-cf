...
def train(args, data, show_loss, show_topk):
    ...
    entity_matrix, relation_matrix = data[7], data[8]

    model = KGCN(args, n_user, n_entity, n_relation, entity_matrix, relation_matrix)
...
def get_feed_dict(model, data, start, end):
    feed_dict = {model.user_index: data[start:end, 0],
                 model.item_index: data[start:end, 1],
    ...
    for user in user_list:
        ...
        if start < len(test_item_list):
            items, scores = model.get_scores(
                sess, {model.user_index: [user] * batch_size,
                       model.item_index: test_item_list[start:] + [test_item_list[-1]] * (
